...
// If token is digit
      	...
      	if(isDigit)
      	{
        	...
        	char *emoji[] = {"LAUGH","HEART","SMILE"};
		char *unicode[] = {LAUGH,HEART,SMILE};
...
//if client is active then send msg otherwise send reciver client is not active to sender client
        	...
        	{ 
           		sprintf(output, "\033[1;32m [msg] From Client %d : %s\033[1:0m", index+1, sep);  //sending the message to that client

          		if(send(Client[id].socketID, output, strlen(output), 0) < 0)
          		{
            			perror("\033[1;32m[--] Can't send message...\033[1;0m\n");
          		}
  
          		memset(output,'\0',sizeof(output));
          		sprintf(output, "\033[1;34m[res] Message sent to client %d...\033[1;0m\n", id+1);  // server response to the client
			printf(" ..... Successfully send the message to client %d from client %d .....\n", id+1, index+1);
        	}
 
        	else
        	{
          		sprintf(output, "\033[1;34m[res] Cannot send the message since client %d is DE-ACTIVE...\033[1;0m\n", id+1);
        	}
      	}
...
// If token is not digit      	
      	...
      	{
        	strcpy(output, "\033[1;34m[res] WRONG CLIENT ID...\033[1;0m\n");
      	}
